...
export default function CreditsPurchase() {
  const [selectedPack, setSelectedPack] = useState(PackageId.Large);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-2xl font-bold items-center gap-2">
      ...
      <CardContent>
        <RadioGroup
          onValueChange={(value) => setSelectedPack(value as PackageId)}
          value={selectedPack}
        >
          {CreditsPackage.map((pack) => (
            <div
              key={pack.id}
              className="flex items-center space-x-3 bg-secondary/40 rounded-md p-3 hover:bg-secondary/80"
